...
function addCode() {
	...
	notetaker.on("click",function() {
		if(notetakerDiv.css("display") === "none") {
            ...
            notetakerDiv.slideDown();
		}
		else {
			//notetakerDiv.css("display","none");
            notetakerDiv.slideUp();
		}
	}.bind(notetakerDiv));
	if (touchSupported) {
...
function addDo() {
    ...
    var vagaband = $("<h2></h2>");
    vagaband.addClass("projectTitle");
	vagaband.append("vagaband");
	vagaband.on("click tap",function() {
		url = "http://www.cindyzeng.com/thevagaband";
		window.open(url,"_blank");
	})

	projectList.append(vagaband);
	projectList.append(divider);
