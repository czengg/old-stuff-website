...
<?php if ( ! empty($comments_by_type['comment']) ) : ?>
                ...
                <div id="comments-list" class="comments">
                    <h3><?php printf($comment_count > 1 ? __('<span>%d</span> Comments', 'hbd-theme') : __('<span>1</span> Comment', 'hbd-theme'), $comment_count) ?></h3>
...
<?php if ( get_option('comment_registration') && !$user_ID ) : ?>
                    <p id="login-req"><?php printf(__('You must be <a href="%s" title="Log in">logged in</a> to post a comment.', 'hbd-theme'),
...
<?php if ( $user_ID ) : ?>
                            <p id="login"><?php printf(__('<span class="loggedin">Logged in as <a href="%1$s" title="Logged in as %2$s">%2$s</a>.</span> <span class="logout"><a href="%3$s" title="Log out of this account">Log out?</a></span>', 'hbd-theme'),
...
<?php endif /* if ( $user_ID ) */ ?>
              ...
              <div id="form-allowed-tags" class="form-section">
                  <p><span><?php _e('', 'hbd-theme') ?></span> <code><?php allowed_tags(); ?></code></p>
              </div>
 
<?php do_action('comment_form', $post->ID); ?>
 
                            <div class="form-submit"><input id="submit" name="submit" type="submit" value="<?php _e('Post Comment', 'hbd-theme') ?>" tabindex="7" /><input type="hidden" name="comment_post_ID" value="<?php echo $id; ?>" /></div>
